<div>
  <button class="w-full bg-blue-700 text-white peer-hover:bg-blue-500 px-4 py-2 my-2 rounded" wire:click="$set('openModalMedicine',true)">{{ __('add medicine') }}</button>
  <x-jet-dialog-modal wire:model="openModalMedicine" >
    <x-slot name="title">
    ...
    <x-slot name="content">
        <div class="grid grid-cols-2">
            ...
            <div class="seleccionadas">
                @foreach ($user_medicines as $key=>$us)
                    <div>
    ...
    <x-slot name="footer">
        <button class="bg-yellow-500 text-white hover:bg-red-400 px-4 py-2 rounded" wire:click="$set('openModalMedicine',false)">
            {{ __('cancel') }}
        </button>
